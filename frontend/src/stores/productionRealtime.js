...
import { normalizeWorkspaceEvent } from '../features/production-dashboard/utils/productionWorkspaceSync.js'
...
export const useProductionRealtimeStore = defineStore('productionRealtime', () => {
  ...
  const connectForUser = (userId) => {
    ...
    activeChannel.listen('.production.workspace.sync', (payload) => {
      lastWorkspaceEvent.value = normalizeWorkspaceEvent(payload)
    })
  ...
  const initializeForUser = async (user) => {
    if (!user?.user_id || user?.role !== 'production') {
      reset()
      return null
    }
